...
def save_statistics_of_path(path, out_path, device=None, batch_size=50, dims=2048, num_workers=8, inception_ckpt_path=None):
    if device is None:
        device = torch.device('cuda' if (torch.cuda.is_available()) else 'cpu')
    else:
        device = torch.device(device)
    block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims]
    model = InceptionV3([block_idx], inception_ckpt_path=inception_ckpt_path).to(device)
    m1, s1 = compute_statistics_of_path(path, model, batch_size, dims, device, num_workers)
    np.savez(out_path, mu=m1, sigma=s1)


def calculate_fid_given_paths(paths, device=None, batch_size=50, dims=2048, num_workers=8, inception_ckpt_path=None):
